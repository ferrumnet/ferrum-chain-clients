...
test('get balances ', async function() {
    const client = ethereumClientForProd();
    ...
    const balancesF = curs.map(currency => client.getBalance(address, currency).then(
        balance => ({ balance, address, currency})));
    const balances = await Promise.all(balancesF);
